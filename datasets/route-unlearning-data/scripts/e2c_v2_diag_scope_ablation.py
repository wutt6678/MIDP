#!/usr/bin/env python3
"""E2C-v2 diagnostic: trainable-scope ablation for M1 I2N binding.
...
SCOPE_REGEXES: dict[str, list[str]] = {
    "S0": [
        r"^model\.language_model\.layers\.\d+\.self_attn\.[qkvo]_proj$",
    ],
    "S1": [
...
def main():
    ...
    parser.add_argument("--scope", required=True, choices=["S0", "S1", "S2", "S3"])
    parser.add_argument("--rank", type=int, default=8,
                        help="LoRA rank (alpha fixed at 2x rank)")
    parser.add_argument("--out-base", default="e2c_v2/outputs/diag_scope")
    parser.add_argument("--run-tag", default="",
                        help="suffix for the output subdir, e.g. _fullscope")
    ...
    rank = args.rank
    alpha = 2 * rank
    subdir = f"{scope}{args.run_tag}"
    output_dir = Path(args.out_base) / subdir
    ...
    model = adapter.attach_unlearning_adapter(
        model, lora_rank=rank, lora_alpha=alpha, lora_dropout=0.05,
    ...
    results: dict[str, Any] = {"scope": scope, "lora_rank": rank,
                               "lora_alpha": alpha, "steps": STEPS,
                               "trainable_parameters": trainable,
                               "n_target_modules": len(target_modules),
