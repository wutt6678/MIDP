...
def _dirty_tracked_code():
    ...
    blame this panel for edits made by the parallel runs.  Same fix as the
    rule-generalization and method-baseline runners.
    """
    missing = [p for p in PP_CODE if not Path(p).exists()]
    if missing:
        return [f"<declared executed code missing: {p}>" for p in missing]
    try:
        out = subprocess.check_output(
            ["git", "status", "--porcelain", "--untracked-files=no", *PP_CODE],
            text=True)
