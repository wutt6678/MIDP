#!/usr/bin/env python3
"""Package compact canary evidence bundles (P0-EVIDENCE-01).

Reads run_manifest.json and run_binding.json from a canary output
directory and copies every referenced artifact into the compact
evidence directory.
...
def _package_model(model_key: str) -> tuple[bool, list[str]]:
    """Package evidence for one model. Returns (pass, errors)."""
    canary_dir = PROJECT_ROOT / "outputs" / "experiments" / "unlearning" / model_key
    evidence_dir = PROJECT_ROOT / "outputs" / "experiments" / "canary_evidence" / model_key

    # Find the canary output directory
    canary_seed = canary_dir / "candidate_margin_v1" / "canary_seed_17"
    if not canary_seed.is_dir():
        return False, [f"Canary output not found: {canary_seed}"]
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Package canary evidence")
    ...
    for model_key in models:
        passed, errors = _package_model(model_key)
